...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    for(int i=0; i < numSlots; i++)
    {
        if (elems[h%numSlots].type==SlotType::EMPTY || elems[h%numSlots].type==SlotType::TOMBSTONE)
        {
            elems[h%numSlots].value = elem;
            elems[h%numSlots].type = SlotType::FILLED;
            numElements++;
            return true;
        }
        h++;
...
bool LinearProbingHashTable::contains(const string& elem) const {
    ...
    for(int i =0; i < numSlots; i++)
    {
        if(elems[h%numSlots].type ==SlotType::EMPTY)
        {
            return false;
        }
        if(elems[h%numSlots].value == elem && elems[h%numSlots].type == SlotType::FILLED)
        {
            return true;
        }
        h++;
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    for(int i =0; i < numSlots;i++)
    {
        if(elems[h%numSlots].value == elem and elems[h%numSlots].type==SlotType::FILLED)
        {
            elems[h%numSlots].type=SlotType::TOMBSTONE;
            numElements--;
            return true;
        }
        h++;
...
void LinearProbingHashTable::printDebugInfo() const {
    for(int i =0; i < hashFn1.numSlots(); i++)
    {
