...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    for(int i = 0; i< numSlots; i++)
    {
          if(elems[h%numSlots].distance==EMPTY_SLOT)
          {
              elems[h%numSlots].element = elem1;
              elems[h%numSlots].distance= steps;
              numElements++;
              return true;
          }
          if(elems[h%numSlots].distance!=EMPTY_SLOT && elems[h%numSlots].distance< steps)
          {
              temp = elems[h%numSlots];
              elems[h%numSlots].element = elem1;
              elems[h%numSlots].distance = steps;
              elem1 = temp.element;
              steps = temp.distance;
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    for(int i = 0; i < numSlots; i++)
    {
        if(elems[h%numSlots].distance < steps)
        {
            return false;
        }
        if(elems[h%numSlots].element==elem || elems[h%numSlots].distance==EMPTY_SLOT)
        {
            return true;
        }
        steps++;
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    for(int i = 0; i < numSlots; i++)
    {
        ...
        if(elems[h%numSlots].element ==elem)
        {
            elems[h%numSlots].distance = EMPTY_SLOT;
            numElements--;
            while(elems[(h+1)%numSlots].distance!=0 && elems[(h+1)%numSlots].distance!=EMPTY_SLOT)
            {
                elems[h%numSlots].element = elems[(h+1)%numSlots].element;
                elems[h%numSlots].distance = elems[(h+1)%numSlots].distance-1;
                elems[(h+1)%numSlots].distance = EMPTY_SLOT;
                h++;
            }
            return true;
        }
