...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Grid<bool> floodArea(terrain.numRows(), terrain.numCols());
    Queue<GridLocation> floods;

    for(GridLocation gl: sources)
    {
        if(terrain.get(gl.row, gl.col)<=height)
        {
            floodArea.set(gl.row, gl.col, true);
            floods.enqueue(gl);
    ...
    {
        GridLocation g = floods.dequeue();

        int r = g.row;
        int c = g.col;

        if(r+1 >=0 && r+1 < terrain.numRows())
        {
            if(terrain.get(r+1, c)<=height && floodArea.get(r+1, c) ==false )
            {
                floodArea.set(r+1,c, true);
                floods.enqueue(GridLocation(r+1,c));
            }
        }
        if(r-1 >=0 && r-1 < terrain.numRows())
        {
        ...
        if(c+1 >=0 && c+1 < terrain.numCols())
        {
            if(terrain.get(r, c+1)<=height && floodArea.get(r, c+1) ==false )
            {
                floodArea.set(r, c+1, true);
                floods.enqueue(GridLocation(r, c+1));
            }
        }
        if(c-1 >= 0 && c-1 < terrain.numCols())
        {
