...
void deleteNucleotides(Nucleotide* dna) {
    ...
    while(dna!=nullptr)
    {
        Nucleotide *n = dna->next;
        //Deletes the whole list, but it is stored in n
        delete dna;
        dna = n;
    }
}
...
Nucleotide* toStrand(const string& str) {
    ...
    {
        Nucleotide* d = new Nucleotide;
        Nucleotide* newStrand = strand;
        d-> value = str[i];
        d-> next = nullptr;

        while(newStrand->next!=nullptr)
        {
            newStrand = newStrand->next;
        }
        newStrand -> next = d;
    }
    return strand;
