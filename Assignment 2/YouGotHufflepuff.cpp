...
Question randomQuestionFrom(Set<Question>& questions) {
    ...
    if (questions.size() ==0)
    {
        error("All the questions have been asked!");
    }
...
Map<char, int> scoresFrom(const Map<Question, int>& answers) {
    ...
    {
        int f = 0;
        for(char c: q.factors)
        {
            if(q.factors.containsKey(c))
            {
                f = q.factors[c];
                if(answers[q]==1)
                {
...
Map<char, double> normalize(const Map<char, int>& scores) {
    /* TODO: Delete this line and the next two, then implement this function. */
    Map<char, double> normalScore;
    double sum = 0.0;
    for(char c: scores)
    {
        sum+=scores[c]*scores[c];
    }
    if(sum ==0)
    {
        error("Values are all 0");
    }
    sum=sqrt(sum);

    for(char c: scores)
    {
        normalScore[c]= scores[c]/sum;
    }
...
Person mostSimilarTo(const Map<char, int>& scores, const Set<Person>& people) {
    ...
    Map<char, double> userNormal = normalize(scores);
    double cValue = -1.0;
    double maxC = -1.0;
    Person winner;
    if(people.isEmpty())
    {
        error("There are no people in the Set");
    }
    for(Person p: people)
    {
        cValue = cosineSimilarityOf(userNormal, normalize(p.scores));
        if(cValue > maxC)
        {
            maxC = cValue;
            winner = p;
